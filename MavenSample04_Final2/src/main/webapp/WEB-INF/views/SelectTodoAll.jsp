				...
				<c:if test="${todo.do_Status=='해야 할 일'}">
					<div class="list-group-item" draggable="true">
						<form action="todoDel.do" method="post">
							...
							${todo.do_endTime} <input type="hidden" id="status"
								value="해야 할 일"> <input type="hidden" id="todoIdx"
								value="${todo.todoIdx}"> <input type="hidden" name="txt"
								value="${todo.content}"> <input type="submit" value="삭제">
						</form>
					</div>
				</c:if>
